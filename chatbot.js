...
(function () {
    ...
    const countryCodes = [
        ...
        { code: '+1784', country: 'vc', country_name: 'Saint Vincent and the Grenadines' },
    ...
    const chatHTML = `
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-audio-lines"><path d="M2 10v3"/><path d="M6 6v11"/><path d="M10 3v18"/><path d="M14 8v7"/><path d="M18 5v13"/><path d="M22 10v3"/></svg>                 </button>
            ...
            <img src="https://demoserver3.sgp1.digitaloceanspaces.com/uploads/images/original/${localStorage.getItem('agent_image')}" alt="Agent" style="width: 26px; height: 26px; border-radius: 50%; margin-right: 8px; object-fit: cover;">
            <span></span>
            <span></span>
            <span></span>
            </div>
    ...
    async function playInitialMessage() {
        try {
            const response = await fetch(
                `https://api.deepgram.com/v1/speak?model=${localStorage.getItem('agent_voice')}`,
                {
                    method: "POST",
                    headers: {
    ...
    async function processTranscript(transcript) {
        ...
        try {
            ...
            const apiResponse = await fetch('https://dev.qixs.ai:3003/knowledge-base/landing/page/ai', {
                method: 'POST',
    ...
    function getApiKeyFromScript() {
        ...
        const getAgentandScriptDetails = async (agentId) => {
            try {
                ...
                localStorage.setItem('call_first_message', response?.data?.call_first_message);
                localStorage.setItem('call_prompt', response?.data?.call_prompt);
                localStorage.setItem('chat_first_message', response?.data?.chat_first_message);
    ...
    class ChatBot {
        ...
        setupFormListeners() {
            ...
            if (form) {
                form.addEventListener('submit', async (e) => {
                    ...
                    const selectedCode = document.getElementById('selectedCode').innerText;
        ...
        addMessage(message, isUser, chat_id = null) {
            ...
            messageDiv.className = `ai_message ${isUser ? 'user-message' : 'bot-message'}`;
        ...
        async endConversation() {
            ...
            if (callScreen.style.display === 'flex' || callScreen.style.display === 'block') {
                return;
            }
